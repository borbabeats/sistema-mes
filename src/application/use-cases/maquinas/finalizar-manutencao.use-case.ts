...
export class FinalizarManutencaoUseCase {
  ...
  async execute(id: number, manutencaoData: FinalizarManutencaoData): Promise<{ maquina: Maquina; manutencao: any }> {
    ...
    const manutencoesEmAndamento = await this.manutencoesRepository.findByStatus(StatusManutencao.EM_ANDAMENTO);
    const manutencaoAtiva = manutencoesEmAndamento.find(m => m.maquinaId === id);
    
    if (!manutencaoAtiva) {
      throw new Error('Nenhuma manutenção em andamento encontrada para esta máquina');
    }

    // Finalizar manutenção
    const updatedManutencao = await this.manutencoesRepository.update(manutencaoAtiva.id, {
