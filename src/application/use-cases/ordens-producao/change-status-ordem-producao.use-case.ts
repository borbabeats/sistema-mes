...
import { ORDENS_PRODUCAO_REPOSITORY_TOKEN, ORDEM_PRODUCAO_STATUS_LOG_REPOSITORY_TOKEN } from '../../../modules/ordens-producao/constants';
import { ChangeStatusOrdemProducaoDto } from '../../../presentation/dto/ordens-producao/change-status-ordem-producao.dto';
import { canTransition, getTransition } from '../../../domain/state-machines/ordem-producao-state-machine';
...
export class ChangeStatusOrdemProducaoUseCase {
  ...
  private validateTransition(ordem: OrdemProducao, novoStatus: StatusOP): void {
    ...
    if (novoStatus === StatusOP.FINALIZADA && ordem.quantidadeProduzida < ordem.quantidadePlanejada) {
    ...
    if (novoStatus === StatusOP.EM_ANDAMENTO && ordem.status !== StatusOP.PLANEJADA) {
      throw new BadRequestException('Só é possível iniciar produção a partir de uma OP PLANEJADA');
    }

    // Só pode retomar se estiver PAUSADA
    if (novoStatus === StatusOP.EM_ANDAMENTO && ordem.status === StatusOP.PAUSADA) {
      // Permitido (retomar)
    } else if (novoStatus === StatusOP.EM_ANDAMENTO && ordem.status !== StatusOP.PLANEJADA) {
      throw new BadRequestException('Só é possível iniciar/retomar produção a partir de PLANEJADA ou PAUSADA');
    }

    // Exemplo: só pode cancelar se não estiver FINALIZADA
    if (novoStatus === StatusOP.CANCELADA && ordem.status === StatusOP.FINALIZADA) {
      throw new BadRequestException('Não é possível cancelar uma OP já finalizada');
    }
  }
}
