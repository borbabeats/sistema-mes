...
export class FinalizarProducaoUseCase {
  ...
  async execute(id: number): Promise<OrdemProducao> {
    ...
    if (ordemProducao.status !== StatusOP.EM_ANDAMENTO && ordemProducao.status !== StatusOP.PAUSADA) {
      throw new Error('Apenas ordens de produção em andamento ou pausadas podem ser finalizadas');
    }
