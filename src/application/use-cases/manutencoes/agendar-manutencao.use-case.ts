...
import { Manutencao, StatusManutencao, TipoManutencao } from '../../../domain/entities/manutencao.entity';
...
export class AgendarManutencaoUseCase {
  ...
  async execute(data: AgendarManutencaoData): Promise<{ maquina: Maquina; manutencao: Manutencao }> {
    ...
    const manutencoesAgendadas = await this.manutencoesRepository.findAgendadas();
    const conflito = manutencoesAgendadas.find(m => 
      m.maquinaId === data.maquinaId && 
      m.dataAgendada.toDateString() === data.dataAgendada.toDateString()
    );

    if (conflito) {
      throw new Error('Já existe manutenção agendada para esta máquina nesta data');
    }
