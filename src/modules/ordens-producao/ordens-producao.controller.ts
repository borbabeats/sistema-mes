...
export class OrdensProducaoController {
  ...
  create(
    @Body() createOrdemProducaoDto: CreateOrdemProducaoDto,
  ...
  @ApiOperation({
    summary: 'Listar todas as ordens de produção com filtros e paginação',
  })
  @ApiResponse({
    status: 200,
  ...
  findAll(
    @Query() filters: FindAllOrdensProducaoDto,
  ...
  @ApiResponse({
    status: 200,
    description: 'Ordem de produção encontrada',
    type: OrdemProducao,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Ordem de produção atualizada',
    type: OrdemProducao,
  })
  @ApiResponse({ status: 400, description: 'Dados inválidos' })
  @ApiResponse({ status: 404, description: 'Ordem de produção não encontrada' })
  ...
  @Roles(Role.ADMIN, Role.GERENTE, Role.OPERADOR)
  @ApiOperation({
    summary: 'Mudar status de uma ordem de produção (state machine)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: 'Transição inválida ou não permitida',
  })
  @ApiResponse({ status: 404, description: 'Ordem de produção não encontrada' })
  async changeStatus(
    @Param('id', ParseIntPipe) id: number,
  ...
  @ApiResponse({
    status: 400,
    description: 'Não é possível atualizar a produção',
  })
  @ApiResponse({ status: 404, description: 'Ordem de produção não encontrada' })
  atualizarProducao(
    @Param('id', ParseIntPipe) id: number,
