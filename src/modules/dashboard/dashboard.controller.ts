...
export class DashboardController {
  ...
  @ApiResponse({ status: 200, description: 'KPIs de produção obtidos com sucesso', type: KpisProducaoDto })
  ...
  @ApiResponse({ status: 200, description: 'KPIs de qualidade obtidos com sucesso', type: KpisQualidadeDto })
  ...
  @ApiResponse({ status: 200, description: 'KPIs de recursos obtidos com sucesso', type: KpisRecursosDto })
  ...
  @ApiResponse({ status: 200, description: 'KPIs de prazos obtidos com sucesso', type: KpisPrazosDto })
  ...
  @ApiQuery({ name: 'dias', required: false, type: Number, description: 'Número de dias para análise (padrão: 30)' })
  @ApiResponse({ status: 200, description: 'Dados do gráfico de produção diária obtidos com sucesso', type: [GraficoProducaoDiariaDto] })
  async getGraficoProducaoDiaria(@Query('dias') dias: number = 30): Promise<GraficoProducaoDiariaDto[]> {
  ...
  @ApiQuery({ name: 'periodo', required: false, type: String, description: 'Período de análise (hoje, semana, mes, ano)' })
  @ApiResponse({ status: 200, description: 'Dados do gráfico de produção por setor obtidos com sucesso', type: [GraficoProducaoSetorDto] })
  async getGraficoProducaoPorSetor(@Query('periodo') periodo: string = 'mes'): Promise<GraficoProducaoSetorDto[]> {
  ...
  @ApiResponse({ status: 200, description: 'Dados do gráfico de status das OPs obtidos com sucesso', type: [GraficoStatusOpsDto] })
  ...
  @Roles(Role.ADMIN, Role.GERENTE, Role.OPERADOR)
  @ApiOperation({ summary: 'Obter dados para gráfico de tendência de qualidade' })
  @ApiQuery({ name: 'dias', required: false, type: Number, description: 'Número de dias para análise (padrão: 30)' })
  @ApiResponse({ status: 200, description: 'Dados do gráfico de tendência de qualidade obtidos com sucesso' })
  ...
  @ApiResponse({ status: 200, description: 'OEE em tempo real obtido com sucesso', type: OeeTempoRealDto })
  ...
  @ApiQuery({ name: 'periodo', required: false, type: String, description: 'Período de análise (hoje, semana, mes, ano)' })
  @ApiResponse({ status: 200, description: 'Top 5 produtos obtidos com sucesso', type: [TopProdutosDto] })
  async getTopProdutos(@Query('periodo') periodo: string = 'mes'): Promise<TopProdutosDto[]> {
  ...
  @ApiQuery({ name: 'dias', required: false, type: Number, description: 'Número de dias para análise (padrão: 7)' })
  @ApiResponse({ status: 200, description: 'Heatmap de produção por turno obtido com sucesso', type: [ProducaoPorTurnoDto] })
  async getProducaoPorTurno(@Query('dias') dias: number = 7): Promise<ProducaoPorTurnoDto[]> {
  ...
  @ApiResponse({ status: 200, description: 'Alertas críticos obtidos com sucesso', type: AlertasCriticosDto })
  ...
  @ApiResponse({ status: 200, description: 'Metas do dia obtidas com sucesso', type: MetasDiaDto })
  ...
  @ApiQuery({ name: 'periodo', required: false, type: String, description: 'Período de análise (hoje, semana, mes)' })
  @ApiResponse({ status: 200, description: 'Ranking de eficiência obtido com sucesso', type: [EficienciaOperadoresDto] })
  async getEficienciaOperadores(@Query('periodo') periodo: string = 'semana'): Promise<EficienciaOperadoresDto[]> {
